...
namespace Attendance
{
    public static class Utility
    {
        ...
        public static DateTime[] GetDates(string year, string month)
        {
            ...
            DateTime dtUserDate = Utility.GetIndianTime();
            if (dtToDate.Date >= dtUserDate.Date)
                dtToDate = dtUserDate.Date.AddDays(-1);
        ...
        public static List<SelectListItem> GetMonths()
        {
            ...
            {
                for (int i = 1; i <= 12; i++)
                    Months.Add(new SelectListItem { Text = CultureInfo.CurrentCulture.DateTimeFormat.GetMonthName(i), Value = i.ToString(), Selected = dtCurrentDateTime.AddMonths(1).Month == i });
            }
            return Months;
        }
